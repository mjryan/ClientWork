/*
* amway.about.js
* Javascript for the About Amway section.
* 
* Copyright Amway
*/
...
amway.about = (function() {
    // private

    function init() {
        initScrollPanels();
        initFilmStrip();
        initGlobalCountryBrowser();
	...
	if (document.getElementById("subMenu1") ==null && document.getElementById("subMenu2") ==null) 
	{		
		$('div.MC_LYT_ScrollPanel').each(function() {
	...
	/*For Highlighting Select Node in Left Navigation*/
	function leftNavigationHighlight()
	{		
		$('div.rtSelected').each(function() {
			$(this).parent().addClass('SN_NAV_Selected')
			});
		
		}
	
    function initFilmStrip() {
        $('#MC_LYT_AboutFilmstrip li').click(function(ev) {
            ev.preventDefault();

            // get the target li clicked
            var $clickedLi = $(this);
            while (!$clickedLi.is('li')) {
                $clickedLi = $clickedLi.parent();
            }

            // get the content container div
            var $container = $('#LYT_Body > .MC_LYT_Container');

            // remove current selected class
            var containerClass = $container.attr('className');
            var selectedClass = containerClass.match(/\bMC_LYT_Selected[\w-]*\b/);
            $container.removeClass(selectedClass);

            // add selected class of clicked item
            var newSelectedClass = ($clickedLi.attr('className').match(/\bMC_LYT_Select[\w-]*\b/) + '').replace('MC_LYT_Select', 'MC_LYT_Selected');
            $container.addClass(newSelectedClass);
...
function resetFlashVideo(movieName){
    var flashMovie = getFlashMovieObject(movieName);
    flashMovie.resetVideo();
}

	function Showvideo()
	{		
		$('div.ourStory div.MC_LYT_static div.MC_IMG_Image').click(function() {
			$('div.videoPlayerContainer').addClass('showVideoPlayer')
			});		
	}
	function hideVideo()
	{		
		$('span.videoPlayerClose').click(function() {
		$('div.videoPlayerContainer').removeClass('showVideoPlayer')
		var elements = document.getElementById('videoMainPlayer').getElementsByTagName("*");
		for(var i=0; i< elements.length;i++)
		{
			if(elements[i].tagName == "OBJECT")
			{
				resetFlashVideo(elements[i].id);
				break;
			}
		}
		});	
    ...
    function initGlobalCountryBrowser() {
        
        $('div.LYT_GlobalFamily div.MC_LYT_Countries li a').click(function(ev) {
            ev.preventDefault();

            var $link = $(this);

            var $countryContainer = $link.parents('div.MC_LYT_CountriesWrap');

            // unselect the currently selected country
            var $currentCountryLink = $('li.MC_LYT_Selected', $countryContainer);
            if ($currentCountryLink.length>0) {
                $currentCountryLink.removeClass('MC_LYT_Selected');
            }
            var $currentCountryInfo = $('div.MC_LYT_CountryInfo.MC_LYT_Selected', $countryContainer);
            if ($currentCountryInfo.length>0) {
                $currentCountryInfo.removeClass('MC_LYT_Selected');
            }

            // select the country clicked
            $link.parent().addClass('MC_LYT_Selected');
            var href = $link.attr('href');
            var newCountryName = href.substring(href.lastIndexOf('#') + 1);
            var $newCountryInfo = $('div.MC_LYT_CountryInfo.MC_LYT_' + newCountryName, $countryContainer);
            if ($newCountryInfo.length>0) {
                $newCountryInfo.addClass('MC_LYT_Selected');
            }
...
jQuery(document).ready(amway.about.init);
...
mDv.sY.width="100px";mDv.sY.height="435px";mDv.sY.top="0px";mDv.sY.left="0px";
