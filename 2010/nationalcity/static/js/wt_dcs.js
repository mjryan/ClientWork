...
/*
     FILE ARCHIVED ON 1:38:06 Jul 13, 2010 AND RETRIEVED FROM THE
	...
	if (arr.length%2==0){
		for (var i=0;i<arr.length;i+=2){
			...
			if (arr[i].indexOf('WT.')==0){
				WT[arr[i].substring(3)]=arr[i+1];
			}
			else if (arr[i].indexOf('DCS.')==0){
				DCS[arr[i].substring(4)]=arr[i+1];
			}
			else if (arr[i].indexOf('DCSext.')==0){
				DCSext[arr[i].substring(7)]=arr[i+1];
			}
		}
